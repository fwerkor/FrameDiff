...
def _iteration_payload(iter_dir: Path) -> dict[str, Any]:
    ...
    baseline_alignment = _baseline_alignment(iter_dir)
    return {
        "iteration": iteration,
        ...
        "baseline_align": _component_state(status, "BASELINE_ALIGN"),
...
def _write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        "| iter | status | baseline aligned | PTA loss | MSA loss | |PTA-MSA| | MSA perturb | |MSA-MSA'| | trace tensors | trace weights |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for item in payload.get("iterations", []):
        trace = item.get("trace", {}) if isinstance(item.get("trace"), dict) else {}
        lines.append(
            "| {iteration} | {status} | {baseline_aligned} | {pta_loss} | {msa_loss} | {pta_msa_abs_loss_delta} | "
...
<body>
  ...
  <table>
    <thead><tr><th>iter</th><th>status</th><th>baseline aligned</th><th>PTA loss</th><th>MSA loss</th><th>|PTA-MSA|</th><th>MSA perturb</th><th>|MSA-MSA'|</th><th>trace tensors</th><th>trace weights</th></tr></thead>
