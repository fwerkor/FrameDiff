...
def emit_task_debug(iteration, final_output, loss):
    ...
    trace_loss("task_final_loss", loss, extra={"source": "msa.emit_task_debug", "iteration": iteration})
...
if __name__ == "__main__":
    # parse arguments
    # module = "../frame_diff_common/model_configs/baichuan.yaml"
    # node_num = 1
    # config_dir = "model_config"
    # rounds = 2
    # mutating_num = 2
    ...
    if args.rank == 0 and os.path.exists(file_path):
        try:
            with open(file_path, "r", encoding="utf-8") as handle:
                trace_event("mutation_record", {"path": file_path, "record": json.load(handle)})
        except Exception as exc:
            trace_event("mutation_record_error", {"path": file_path, "error": str(exc)})
    ...
    try:
        ...
        for step_idx in range(train_iters):
            ...
            trace_event("msa_forward_step_begin", {"iteration": iteration, "step": step_idx + 1})
            ...
            trace_tensor(
                ...
                "step_final_output",
                final_output,
                stage="msa_step_output",
                extra={"iteration": iteration, "step": step_idx + 1},
            )
            trace_loss(
                "overall_loss",
                loss.detach(),
                extra={"backend": "msa", "iteration": iteration, "step": step_idx + 1},
            )
            if step_idx == 0:
    ...
    if args.rank == 0:
        ...
        trace_event(
            "msa_runtime_end",
            {
                ...
                "memory_mb": mem_usage,
                "logged_loss": float(final_logged_loss),
            },
        )
