...
def create_mutated_graph_with_random_configs(configs, selected_files, num_decoder_nodes: int = 2,
    ...
    mutating_record = {
        "fullnet_assembly": {
            "enabled": os.getenv("LMSV_FULLNET_ASSEMBLY", "0") == "1",
            "mode": os.getenv("LMSV_FULLNET_ASSEMBLY_MODE", "single_model_fullnet"),
            "decoder_layers": int(num_decoder_nodes),
...
if __name__ == "__main__":
    ...
    if os.getenv("LMSV_FULLNET_ASSEMBLY", "0") == "1" and configs:
        if len(configs) > 1:
            print(f"整网模式启用：使用 {selected_files[0]} 作为所有 decoder 层的基准配置")
        configs = [configs[0]]
