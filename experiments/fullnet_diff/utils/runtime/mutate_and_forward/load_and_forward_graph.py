...
from utils.runtime.fullnet_trace import set_trace_step, trace_event, trace_loss, trace_tensor
...
record_file_name = ""
no_mutating = False
# module = "../frame_diff_common/model_configs/mixtral_decoder.yaml" 
module = args.module
# # module = None
# node_num = 1
...
def emit_task_debug(iteration, final_output, loss):
    ...
    trace_tensor(0, "full_network", "task_final_output", final_output, stage="task_debug_output")
    trace_loss("task_final_loss", loss, extra={"source": "pta.emit_task_debug", "iteration": iteration})
...
if __name__ == "__main__":
    ...
    trace_event(
        "pta_runtime_begin",
        {
            ...
            "shared_weight_mode": os.getenv("LMSV_SHARED_WEIGHT_MODE", ""),
            "perturb": os.getenv("LMSV_FULLNET_PERTURB", "0"),
        },
    )
    ...
    if args.rank == 0 and os.path.exists(json_file_path):
        try:
            with open(json_file_path, "r", encoding="utf-8") as handle:
                trace_event("mutation_record", {"path": json_file_path, "record": json.load(handle)})
        except Exception as exc:
            trace_event("mutation_record_error", {"path": json_file_path, "error": str(exc)})
    ...
    try:
        ...
        for step_idx in range(train_iters):
            ...
            set_trace_step(step_idx)
            torch.npu.reset_peak_memory_stats()
            step_start_time = time.time()
            trace_event("pta_forward_step_begin", {"iteration": iteration, "step": step_idx + 1})
            final_output = model_graph[0].forward(debug=True)
            ...
            trace_tensor(
                ...
                "step_final_output",
                final_output,
                stage="pta_step_output",
                extra={"iteration": iteration, "step": step_idx + 1},
            )
            trace_loss(
                "overall_loss",
                loss.detach(),
                extra={"backend": "pta", "iteration": iteration, "step": step_idx + 1},
            )
    ...
    if args.rank == 0:
        ...
        trace_event(
            "pta_runtime_end",
            {
                ...
                "memory_mb": mem_usage,
                "logged_loss": float(final_logged_loss.item()),
            },
        )
