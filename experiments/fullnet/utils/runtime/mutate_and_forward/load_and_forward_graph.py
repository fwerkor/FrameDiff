...
record_file_name = ""
no_mutating = False
# module = "../model_config/mixtral_decoder.yaml" 
module = args.module
# # module = None
# node_num = 1
    ...
    if args.rank == 0 and os.path.exists(json_file_path):
        try:
            with open(json_file_path, "r", encoding="utf-8") as handle:
                trace_event("variant_record", {"path": json_file_path, "record": json.load(handle)})
        except Exception as exc:
            trace_event("variant_record_error", {"path": json_file_path, "error": str(exc)})
