...
def find_preferred_loss_mismatch(
    ...
    *,
    ...
    if pta_step_csv_path and msa_step_csv_path:
        pta_step_index = _read_loss_series(Path(pta_step_csv_path))
        msa_step_index = _read_loss_series(Path(msa_step_csv_path))
        if sorted(set(pta_step_index) & set(msa_step_index)):
